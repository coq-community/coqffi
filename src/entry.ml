...
let polymorphic_params (decl : type_declaration) : string list =
  ...
  let existing_params params =
    let polymorphic_param (t : type_expr) : string option =
      ...
      | Tvar (Some x) -> Some x
      | _ ->
          failwith "Type parameters should be made with [Tvar] and nothing else"
    in

    minimize (List.filter_map polymorphic_param params)
  in
...
let has_coq_model : attributes -> string option = get_attr_string "coq_model"
...
let prototype_of_constructor params_type args ret =
  ...
  | Cstr_tuple args ->
      ...
      let params_constr =
        List.sort_uniq String.compare (params_ret @ params_constr)
      in
...
let entry_of_value lf lwt_module ident desc loc =
  ...
  if is_pure desc.val_attributes model repr then
    EFunc
      {
        func_name = name;
        func_type = repr;
...
let entry_of_type lf ident decl rec_status loc =
  ...
  let poly_default_type_ret =
    of_mono_type_repr params
      (TParam (CName type_name, List.map (fun x -> TParam (CName x, [])) params))
  in

  let to_variant_entry params v =
    let ret =
      Option.value ~default:poly_default_type_ret
        (Option.map type_repr_of_type_expr v.cd_res)
    in
    {
  ...
  let unify v =
    v.variant_prototype.prototype_ret_type = mono_default_type_ret
  in
...
let unvisited = function Unvisited -> true | _ -> false

let on_stack = function OnStack -> true | _ -> false
...
let find_mutually_recursive_types tl =
  ...
  let matrix : (string, node) Hashtbl.t =
    let tbl = Hashtbl.create input_length in
    List.iter
      (fun t ->
        let deps = dependencies t in
        let min_deps =
          List.filter (fun x -> List.exists (String.equal x) deps) nodes
        in
  ...
  let rec dfs at =
    ...
    List.iter
      (fun t ->
        ...
        if on_stack !(dep.node_state) then
          at.node_low := min !(dep.node_low) !(at.node_low))
      at.node_deps;

    if !(at.node_id) = !(at.node_low) then
      res := List.rev (empty_stack at) :: !res
  in

  Hashtbl.iter (fun _ v -> if unvisited !(v.node_state) then dfs v) matrix;

  List.rev !res

let translate_function ~rev_namespace tbl f =
  { f with func_type = translate_type_repr ~rev_namespace tbl f.func_type }
...
let translate_lwt ~rev_namespace tbl lwt =
  { lwt with lwt_type = translate_type_repr ~rev_namespace tbl lwt.lwt_type }

let translate_prototype ~rev_namespace tbl p =
  let tbl =
    List.fold_left
      (fun tbl x -> Translation.preserve x tbl)
      tbl p.prototype_type_args
  in
  {
    p with
    prototype_args =
...
let translate_exception ~rev_namespace tbl e =
  {
    e with
    exception_prototype =
      translate_prototype ~rev_namespace tbl e.exception_prototype;
  }
