...
let header = {|(* This file has been generated by coqffi. *)
...
let print_type ~param fmt (t : type_info) =
  ...
  fprintf fmt "%a -> %a"
    (print_type_tree ~param:param) t.domain_types
  ...
  begin
    ...
    | Some ident ->
      fprintf fmt "Definition %s : %a := %@%s.\n"
        name
        (print_type ~param:true) t.type_sig
        ident
    | None ->
...
let print_coq_primitive iname fmt p =
  ...
  fprintf fmt "\n| %s : %a"
    pname
    (print_type ~param:false) { p.type_sig with codomain_type = TypeLeaf (iname, [p.type_sig.codomain_type])}
...
let print_coq_extraction_primitive modname fmt (p : primitive_entry) =
  ...
  fprintf fmt "  Axiom (ocaml_%s : %a).\n"
    name
    (print_type ~param:false) p.type_sig;
