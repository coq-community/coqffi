...
let find_duplicates : features -> feature list =
  let rec find_dup dups = function
    | (f, _) :: rst ->
        find_dup (if List.mem_assoc f rst then f :: dups else dups) rst
    | [] -> List.sort_uniq compare dups
  in
  find_dup []
...
let check_features_consistency lwt_alias lf ~wduplicate =
  ...
  let warning fmt feature =
    fprintf fmt "Warning: Feature `%s' has been selected several times.@ "
      (name feature)
  in

  if wduplicate then
    fprintf err_formatter "%a@?" (pp_print_list warning) (find_duplicates lf);

  if is_enabled lf FreeSpec && is_disabled lf Interface then
    raise FreeSpecRequiresInterface;
